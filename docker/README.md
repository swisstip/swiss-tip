...
The images of the Swiss TIP MCP server with `swisstip-mcp` 0.3.3 from PyPI.
...
| --- | --- | --- | --- |
| `swiss-tip-mcp:0.3.3-slim` | [mcp-slim/Dockerfile](mcp-slim/Dockerfile) | the repository root | `swisstip-mcp` 0.3.3 only; the release is mounted on `/srv/swiss-tip` |
| `swiss-tip-mcp:0.3.3` | [mcp/Dockerfile](mcp/Dockerfile) | `docker/mcp` | the slim MCP image, plus CPU-only Ollama 0.34.0 on `127.0.0.1:11434` and `qwen3-embedding:0.6b`; no release |
...
| `swiss-tip-calendar-connector:0.3.3` | [calendar-connector/Dockerfile](calendar-connector/Dockerfile) | the repository root | `swisstip-calendar-connector` 0.3.3 only, the first [dataset connector](../docs/architecture/dataset-connectors.md): HTTP on port 8100 for the MCP server, no dataset; bundles are mounted on `/srv/swiss-tip/datasets`, one subdirectory each |
...
```shell
docker build -f docker/mcp-slim/Dockerfile -t swiss-tip-mcp:0.3.3-slim .
docker build -t swiss-tip-mcp:0.3.3 docker/mcp
docker build -f docker/opencode/Dockerfile -t local/swiss-tip-opencode:latest .
docker build -f docker/calendar-connector/Dockerfile -t local/swiss-tip-calendar-connector:0.3.3 .
...
| `swisstip_mcp_version` | the `swisstip-mcp` version, which is the tag of both MCP images: built here, pulled there; default `0.3.3` |
...
```shell
docker run --rm -p 8000:8000 -v "$PWD/releases/<pack>:/srv/swiss-tip:ro" swiss-tip-mcp:0.3.3-slim
docker run --rm -i -v "$PWD/releases/<pack>:/srv/swiss-tip:ro" swiss-tip-mcp:0.3.3-slim --transport stdio
```
...
docker run -d --name swiss-tip -p 8000:8000 \
  -v "$PWD/releases/<pack>:/srv/swiss-tip:ro" swiss-tip-mcp:0.3.3-slim \
...
```shell
docker run --rm -v "$PWD/releases/<pack>:/kb" swiss-tip-mcp:0.3.3 \
