...
from .datasets import POSTAL_CODE, Dataset, DatasetSource, DatasetType, Period, absent

CALENDAR_REQUIRES = ["postal_code"]
CALENDAR_ACCEPTS = ["start", "end", "limit"]
LookupStatus = Literal["SUPPORTED", "OUT_OF_COVERAGE"]
ConnectorGapDimension = Literal["postal_code_not_covered", "period_not_published", "no_dates_in_range"]
...
def summary_of(dataset: Dataset, issue: str | None = None) -> DatasetSummary:
    ...
    return DatasetSummary(
        ...
        row_count=manifest.row_count, requires=list(CALENDAR_REQUIRES), accepts=list(CALENDAR_ACCEPTS),
...
def calendar_lookup(dataset: Dataset, request: ConnectorLookupRequest) -> ConnectorLookupResponse:
    """The dates of one postal code inside a range, oldest first, at most `limit` of them.

    The range is the request's, clipped to the published period; a range wholly outside it is the
    period_not_published gap, a postal code the rows do not hold the postal_code_not_covered gap, and a covered
    code with no date left in the range the no_dates_in_range gap. Dates are compared as calendar dates: no time
    of day, no time zone.
    """
    manifest = dataset.manifest
    ...
    if request.postal_code not in manifest.postal_codes:
        return answer([], gap=ConnectorGap(
            dimension="postal_code_not_covered", published_values=manifest.postal_codes,
            message=f"{manifest.title}: no dates are published for postal code {request.postal_code}; the dataset "
    ...
    rows = [row for row in dataset.rows if row.postal_code == request.postal_code and start <= row.date <= end]
    if not rows:
        return answer([], gap=ConnectorGap(
            dimension="no_dates_in_range", published_values=published,
            message=f"{manifest.title}: no date for postal code {request.postal_code} between {start.isoformat()} and "
